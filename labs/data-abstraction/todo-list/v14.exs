...
defmodule TodoList do
  ...
  defimpl Enumerable, for: TodoList do
    ...
    def reduce(_todo_list, {:halt, acc}, _fun), do: {:halted, acc}
    def reduce(todo_list, {:suspend, acc}, fun), do: {:suspended, acc, &reduce(todo_list, &1, fun)}
    def reduce([], {:cont, acc}, _fun), do: {:done, acc}
    def reduce([head | tail], {:cont, acc}, fun), do: reduce(tail, fun.(head, acc), fun)
    def reduce(%TodoList{} = todo_list, {:cont, acc}, fun) do
      [head | tail] = todo_list.entries |> Enum.map(&elem(&1, 1))
      reduce(tail, fun.(head, acc), fun)
    end

    def member?(%TodoList{}=todo_list, id) when is_integer(id) do
      {:ok, Map.has_key?(todo_list.entries, id)}
    end
    def member?(%TodoList{}=todo_list, %{id: i1, date: d1, title: t1}=entry) do
      case todo_list.entries[i1] do
        %{id: i2, date: d2, title: t2} ->
          {:ok, i1 == i2 and d1 == d2 and t1 == t2}
        nil ->
          {:ok, false}
      end
    end
...
todo_list
|> Enum.count
|> IO.inspect(label: "\tEnum.count")

todo_list
|> Enum.member?(1)
|> IO.inspect(label: "\tEnum.member?(1)")

todo_list
|> Enum.member?(5)
|> IO.inspect(label: "\tEnum.member?(5)")

todo_list
|> Enum.member?(%{date: ~D[2018-12-19], id: 1, title: "Dentist"})
|> IO.inspect(label: "\tEnum.member?(%{date: ~D[2018-12-19], id: 1, title: \"Dentist\"})")

todo_list
|> Enum.member?(%{date: ~D[2018-12-19], id: 2, title: "Dentist"})
|> IO.inspect(label: "\tEnum.member?(%{date: ~D[2018-12-19], id: 2, title: \"Dentist\"})")

todo_list
|> Enum.map(&Map.get(&1, :id))
|> IO.inspect(label: "\tEnum.map(&Map.get(&1, :id))")

todo_list
|> Enum.map(&Map.get(&1, :title))
|> IO.inspect(label: "\tEnum.map(&Map.get(&1, :title))")
